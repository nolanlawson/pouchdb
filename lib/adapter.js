...
module.exports = function (Pouch) {
  return function (opts, callback) {
    ...
    api.post = function (doc, opts, callback) {
      ...
      if (typeof doc !== 'object' || _.isArray(doc)) {
    ...
    api.removeAttachment = function (docId, attachmentId, rev, callback) {
      api.get(docId, function (err, obj) {
        ...
        if (_.keys(obj._attachments).length === 0) {
    ...
    api.revsDiff = function (req, opts, callback) {
      ...
      function processDoc(id, rev_tree) {
        ...
        _.forEach(missingForId, function (rev) {
          addToMissing(id, rev);
        });
      }
    ...
    function compactDocument(docId, max_height, callback) {
      customApi._getRevisionTree(docId, function (err, rev_tree) {
        ...
        var candidates = [];
        var revs = [];
        _.forEach(_.keys(height), function (rev) {
    ...
    api.compact = function (opts, callback) {
      ...
      api.changes({complete: function (err, res) {
        ...
        _.forEach(res.results, function (row) {
      ...
      if (opts.open_revs) {
        if (opts.open_revs === "all") {
          customApi._getRevisionTree(id, function (err, rev_tree) {
            ...
            leaves = _.map(merge.collectLeaves(rev_tree), function (leaf) {
              return leaf.rev;
            });
            finishOpenRevs();
          });
        } else {
          if (_.isArray(opts.open_revs)) {
      ...
      return customApi._get(id, opts, function (err, result) {
        ...
        if (opts.revs || opts.revs_info) {
          ...
          var path = arrayFirst(paths, function (arr) {
            return _.map(arr.ids, function (x) { return x.id; })
              .indexOf(doc._rev.split('-')[1]) !== -1;
          });

          path.ids.splice(_.map(path.ids, function (x) {return x.id; })
                          .indexOf(doc._rev.split('-')[1]) + 1);
          path.ids.reverse();
          ...
          if (opts.revs) {
            doc._revisions = {
              ...
              ids: _.map(path.ids, function (rev) {
                return rev.id;
              })
            };
          }
          if (opts.revs_info) {
            ...
            doc._revs_info = _.map(path.ids, function (rev) {
              pos--;
              return {
                rev: pos + '-' + rev.id,
        ...
        if (opts.attachments && doc._attachments) {
          ...
          var count = _.keys(attachments).length;
          if (count === 0) {
            return call(callback, null, doc);
          }
          _.forEach(_.keys(attachments), function (key) {
    ...
    api.bulkDocs = function (req, opts, callback) {
      ...
      if (!_.isArray(req.docs)) {
        return call(callback, errors.QUERY_PARSE_ERROR);
      }

      for (var i = 0; i < req.docs.length; ++i) {
        if (typeof req.docs[i] !== 'object' || _.isArray(req.docs[i])) {
          return call(callback, errors.NOT_AN_OBJECT);
        }
      }
    ...
    api.taskqueue.execute = function (db) {
      if (taskqueue.ready) {
        _.forEach(taskqueue.queue, function (d) {
