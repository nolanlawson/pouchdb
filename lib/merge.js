  ...
  _.forEach(tree, function (branch) {
    if (branch.pos === path.pos && branch.ids[0] === path.ids[0]) {
      ...
      while (trees.length > 0) {
        ...
        _.forEach(item.ids[2], function (el, idx) {
...
function stem(tree, depth) {
  ...
  var stemmedPaths = _.map(PouchMerge.rootToLeaf(tree), function (path) {
...
PouchMerge.traverseRevTree = function (revs, callback) {
  ...
  while (toVisit.length > 0) {
    ...
    /*jshint loopfunc: true */
    _.forEach(tree[2], function (branch) {
...
PouchMerge.collectLeaves = function (revs) {
  ...
  _.map(leaves, function (leaf) { delete leaf.pos; });
  return leaves;
};
