...
function IdbPouch(opts, callback) {
  ...
  req.onupgradeneeded = function (e) {
    ...
    if (e.oldVersion < 2) {
      // version 2 adds the total rows store
      runMigration2(e);
    }
  };
  ...
  function runMigration2(e) {
    ...
    docStore.openCursor().onsuccess = function (event) {
      ...
      if (cursor) {
        ...
        metadata.deleted = utils.isDeleted(metadata) ? "1" : "0";
        docStore.put(metadata);
        cursor['continue']();
      } else {
        docStore.createIndex('deleted', 'deleted', {unique : false});
      }
    };
  }
    ...
    function writeDoc(docInfo, callback) {
      ...
      function finish() {
        ...
        dataReq.onsuccess = function (e) {
          ...
          var metadata = utils.extend(true, {deleted : utils.isDeleted(docInfo.metadata) ? "1" : "0"}, docInfo.metadata);
  ...
  function allDocsKeysQuery(totalRows, opts, callback) {
    ...
    if (!keys.length) { // empty list is okay
      ...
      // do a separate "key" query for each key in the keys
          ...
          if (resultsToCollate.length === keys.length) {
            ...
            keys.forEach(function (key) {
              ...
              if (result.res.rows.length) {
                results = results.concat(result.res.rows);
              } else {
  ...
  function allDocsNormalQuery(totalRows, opts, callback) {
    ...
    if (descending && start && end) {
      // unfortunately IDB has a bug (on Chrome at least) where IDBKeyRange.bound is invalid if the
      // start is less than the end in descending mode.  Best bet
  ...
  api._allDocs = function idb_allDocs(opts, callback) {
    ...
    // first count the total_rows using the undeleted count
    var txn = idb.transaction([DOC_STORE], 'readonly');

    var totalRows;
    function countUndeletedDocs(e) {
      totalRows = e.target.result;
    }

    var index = txn.objectStore(DOC_STORE).index('deleted');
    index.count(global.IDBKeyRange.only("0")).onsuccess = countUndeletedDocs;
    ...
    index.get(docId).onsuccess = function (event) {
      ...
      revs.forEach(function (rev) {
        ...
        index.getKey(key).onsuccess = function (e) {
          ...
          count--;
          if (!count) {
            var metadata = utils.extend(true, {deleted : utils.isDeleted(metadata) ? "1" : "0"}, metadata);
...
IdbPouch.destroy = utils.toPromise(function (name, opts, callback) {
  ...
  req.onblocked = idbError(callback);
