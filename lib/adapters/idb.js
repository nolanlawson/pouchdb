...
function IdbPouch(opts, callback) {
  ...
  function allDocsKeysQuery(opts, callback) {
    var keys = opts.keys;
      ...
      keys.forEach(function (key) {
        ...
        api._allDocs(subOpts, function (err, res) {
          ...
          if (resultsToCollate.length === keys.length) {
            ...
            var keysResult = {offset : opts.skip, rows : []};
            for (var i = 0; i < resultsToCollate.length; i++) {
              var result = resultsToCollate[i];
              if (result.err) {
                return utils.call(callback, err);
              } else {
                if (!keysAdded[result.key]) {
                  // duplicate keys are ignored
                  keysAdded[result.key] = true;
                  keysResult.rows = keysResult.rows.concat(result.res.rows);
                }
              }
            }
            keysResult.total_rows = keysResult.rows.length;
            utils.call(callback, null, keysResult);
          }
        });
      });
    }
  ...
  api._allDocs = function idb_allDocs(opts, callback) {
    ...
    if ('keys' in opts) {
      return allDocsKeysQuery(opts, callback);
    }
