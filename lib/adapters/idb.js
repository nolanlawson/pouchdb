...
function IdbPouch(opts, callback) {
  ...
  var name = opts.name;
  var req = global.indexedDB.open(name, POUCH_VERSION);
  ...
  function onDbCreated(e) {
    ...
    console.log('trying to open readwrite, idb.version? ' + idb.version);
    if (idb.setVersion) {
      // old version of spec; onupgradeneeded is not called
      // and readonly/readwrite use integer values
      READ_ONLY = 0;
      READ_WRITE = 1;
      if (idb.version === '') { // not upgraded yet
        console.log('samsung!');
        var req2 = idb.setVersion(POUCH_VERSION);
        req2.onsuccess = function () {
          createSchema(idb);
          onDbCreated({target : {result : idb}});
        };
        req2.onerror = idbError(callback);
        return;
      }
    }
  ...
  api._get = function idb_get(id, opts, callback) {
    ...
    if (opts.ctx) {
      ...
      txn = idb.transaction([DOC_STORE, BY_SEQ_STORE, ATTACH_STORE], READ_ONLY);
    }
  ...
  api._allDocs = function idb_allDocs(opts, callback) {
    ...
    var transaction = idb.transaction([DOC_STORE, BY_SEQ_STORE], READ_ONLY);
  ...
  api._info = function idb_info(callback) {
    var count = 0;
    var update_seq = 0;
    var txn = idb.transaction([DOC_STORE, META_STORE], READ_ONLY);
  ...
  api._getRevisionTree = function (docId, callback) {
    var txn = idb.transaction([DOC_STORE], READ_ONLY);
