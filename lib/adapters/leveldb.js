...
var BY_GROUP_ID_STORE = 'by-sequence';
...
function LevelPouch(opts, callback) {
  ...
  function afterDBCreated() {
    ...
    stores.byGroupIdStore = db.sublevel(BY_GROUP_ID_STORE, {valueEncoding: 'json'});
  ...
  api._bulkDocs = function (req, opts, callback) {
      ...
      function finish() {
        ...
        stores.bySeqStore.put(formatSeq(doc.metadata.seq), doc.data, function (err) {
          stores.docStore.put(doc.metadata.id, doc.metadata, function (err) {
            results.push(doc);
            var groupId = doc.__pouch_group_id;
            if (groupId) {
              var newId = groupId + sep + doc.metadata.id;
              var groupDoc = {
                docId : doc.metadata.id,
                groupId : groupId
              };
              stores.byGroupIdStore.put(newId, groupDoc, function (err) {
                saveUpdateSeq(callback2);
              });
            } else {
    ...
    var groupId = opts.__pouch_group_id;

    if (groupId) {
      // delete everything associated with this groupId
      var groupStream = stores.byGroupId.readStream({
        start : groupId + sep,
        end : groupId + sep + endsep
      });

      var batch = [];

      groupStream.on('data', function (entry) {
        batch.push({
          key : entry.groupId,
          type : 'del',
          prefix : stores.byGroupIdStore
        });
        batch.push({
          key: entry.docId,
          type: 'del',
