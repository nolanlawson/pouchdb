...
var TOTAL_ROWS_STORE = 'total-rows-store';
var VERSION_STORE = 'version-store';
...
function LevelPouch(opts, callback) {
  ...
  function closeStores(callback) {
    ...
    var stores = [
      ...
      path.join(dbpath, TOTAL_ROWS_STORE)
    ];
    var closed = 0;
  ...
  function initstore(store_name, encoding) {
    ...
    function setup_store(err, ldb) {
      ...
      function setAdapterVersion() {
        stores[VERSION_STORE].put('version', ADAPTER_VERSION, function (err) {
          finish();
        });
      }
      ...
      function runMigrations() {
        ...
        // check the adapter version to see if we need to run migrations
        stores[VERSION_STORE].get('version', function (err, value) {
          var adapterVersion = err ? 1 : value;

          if (adapterVersion < 2) {
            // run migration #2, add in the TOTAL_ROWS counts
            var docstream = stores[DOC_STORE].readStream({});
            docstream.on('data', function (entry) {
              ...
              if (!local && !deleted) {
                stores[TOTAL_ROWS_STORE].put(metadata.id, {});
              }
            });
            docstream.on('end', function (err) {
              setAdapterVersion();
            });
          } else {
            setAdapterVersion();
  ...
  fs.stat(opts.name, function (err, stats) {
    function initstores() {
      ...
      initstore(TOTAL_ROWS_STORE, 'json',
      initstore(VERSION_STORE, 'json'));
    }
  ...
  api._bulkDocs = function (req, opts, callback) {
    ...
    function writeDoc(doc, callback2) {
      ...
      function finish() {
        ...
        var deleted = utils.isDeleted(doc.metadata);
        var local = utils.isLocalId(doc.metadata.id);
        // store non-deleted non-local docs for total_rows count
        if (!local) {
          if (deleted) {
            stores[TOTAL_ROWS_STORE].del(doc.metadata.id, function (err) {
              flushDocsToDB();
            });
          } else { // not deleted
            stores[TOTAL_ROWS_STORE].put(doc.metadata.id, {}, function (err) {
              flushDocsToDB();
            });
          }
        }
  ...
  api._allDocs = function (opts, callback) {
    ...
    // first read the total_rows
    var totalRowsStream = stores[TOTAL_ROWS_STORE].createKeyStream({});
    var totalRows = 0;
    totalRowsStream.on('data', function (data) {
      totalRows++;
    });
...
LevelPouch.destroy = utils.toPromise(function (name, opts, callback) {
  ...
  var stores = [
    ...
    path.join(dbpath, ATTACH_BINARY_STORE),
    path.join(dbpath, TOTAL_ROWS_STORE),
    path.join(dbpath, VERSION_STORE)
  ];
  var closed = 0;
  stores.map(function (path) {
    var store = STORES[path];
