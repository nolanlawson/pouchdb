...
function WebSqlPouch(opts, callback) {
  ...
  function runMigration3(tx) {
    tx.executeSql('ALTER TABLE ' + DOC_STORE + ' ADD COLUMN group_id;', [], function () {
      tx.executeSql('CREATE INDEX IF NOT EXISTS \'doc-group-idx\' ON ' +
        DOC_STORE + ' (group_id)');
    });
  }
  ...
  function onGetVersion(tx, dbVersion) {
    if (dbVersion === 0) {
      ...
      var doc = 'CREATE TABLE IF NOT EXISTS ' + DOC_STORE +
        ' (id unique, seq, json, winningseq, local TINYINT(1), group_id)';
  ...
  api._bulkDocs = function (req, opts, callback) {
    ...
    function writeDoc(docInfo, callback, isUpdate) {
      ...
      function finish() {
        ...
        var sql = 'INSERT INTO ' + BY_SEQ_STORE + ' (doc_id_rev, json, deleted, group_id) ' +
          'VALUES (?, ?, ?, ?);';
        var sqlArgs = [
          data._id + "::" + data._rev,
          JSON.stringify(data),
          utils.isDeleted(docInfo.metadata, docInfo.metadata.rev) ? 1 : 0,
          data.__pouch_group_id || null
        ];
    ...
    preprocessAttachments(function () {
      db.transaction(function (txn) {
        ...
        if (groupId) {
          tx.executeSql('DELETE FROM ' + DOC_STORE + ' WHERE group_id = ?', [groupId], function () {
            doBulkDocs();
          });
        } else {
          doBulkDocs();
