...
var BY_SEQ_STORE_DELETED_INDEX = quote('by_seq_deleted_idx');
    ...
    db.transaction(function (tx) {
      ...
      tx.executeSql(meta, [], function () {
        ...
        tx.executeSql('SELECT sql FROM sqlite_master WHERE tbl_name = ' + META_STORE, [], function (tx, result) {
          var createSql = result.rows.item(0).sql;
          if (!/db_version/.test(createSql)) {
            ...
            // column exists, can safely add index
            tx.executeSql('CREATE INDEX IF NOT EXISTS ' + BY_SEQ_STORE_DELETED_INDEX + ' ON ' +
              BY_SEQ_STORE + ' (deleted)');
