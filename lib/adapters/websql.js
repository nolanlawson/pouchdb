...
function webSqlPouch(opts, callback) {
  ...
  function isMangledUnicode(str) {
    ...
    // Rather than browser sniffing, I'm going to hope that testing every odd character for \u0000 is good enough
  ...
  function decodeUtf8(str) {
    ...
    } catch (err) { // URI error in safari, string is already escaped but possibly mangled
      window.console.log('got a URI error, skipping utf8 decoding: ' + err);
      result = str;
    }
    if (isMangledUnicode(result)) {
      return unmangleUnicode(result);
    } else {
      return result;
    }
  }
