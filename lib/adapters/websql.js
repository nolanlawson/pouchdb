...
function webSqlPouch(opts, callback) {
  ...
  function decodeUtf8(str) {
    var result = decodeURIComponent(window.escape(str));
    // Safari is weird, it encodes everything with bonus \u0000 characters
    if (/Safari/.test(window.navigator.userAgent)) {
      result = JSON.parse(JSON.stringify(str).replace(/\\u0000/g, ''));
    }
    return result;
  }
