...
function webSqlPouch(opts, callback) {
  ...
  api._bulkDocs = function (req, opts, callback) {
    ...
    var docInfos = _.map(userDocs, function (doc, i) {
    ...
    function preprocessAttachments(callback) {
      ...
      _.forEach(docInfos, function (docInfo) {
        var attachments = docInfo.data && docInfo.data._attachments ?
          _.keys(docInfo.data._attachments) : [];
    ...
    preprocessAttachments(function () {
      db.transaction(function (txn) {
        tx = txn;
        var ids = '(' + _.map(docInfos, function (d) {
  ...
  function makeRevs(arr) {
    return _.map(arr, function (x) { return {rev: x.rev}; });
  }
  ...
  api._allDocs = function (opts, callback) {
    ...
    var sqlArgs = [];
    if ('keys' in opts) {
      sql += ' WHERE ' + DOC_STORE + '.id IN (' + _.map(opts.keys, function () {
        return '?';
      }).join(',') + ')';
    ...
    db.transaction(function (tx) {
      ...
      if ('keys' in opts) {
        _.forEach(opts.keys, function (key) {
    ...
    db.transaction(function (tx) {
      ...
      tx.executeSql(sql, [docId], function (tx, result) {
        ...
        var sql = 'DELETE FROM ' + BY_SEQ_STORE + ' WHERE doc_id_rev IN (' +
          _.map(revs, function (rev) {return quote(docId + '::' + rev); }).join(',') + ')';
