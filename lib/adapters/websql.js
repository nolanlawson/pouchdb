...
var VERSION_STORE = quote('version');
...
function webSqlPouch(opts, callback) {
  ...
  function runMigration2(tx, onComplete) {
    ...
    var sql = 'SELECT ' + DOC_STORE + '.id AS id, ' + DOC_STORE + '.json AS metadata FROM ' +
    ...
    tx.executeSql(sql, [], function (tx, result) {
      ...
      for (var i = 0; i < result.rows.length; i++) {
        ...
        if (utils.isDeleted(metadata)) {
          ...
          nonDeleted.push(docid);
        }
      }

      tx.executeSql('ALTER TABLE ' + DOC_STORE + ' ADD COLUMN deleted TINYINT(1)', [], function () {
        tx.executeSql('UPDATE ' + DOC_STORE + ' SET deleted = 1 WHERE id IN (' + deleted.map(function () {
          return '?';
        }).join(',') + ')', deleted, function () {
          tx.executeSql('UPDATE ' + DOC_STORE + ' SET deleted = 0 WHERE id IN (' + nonDeleted.map(function () {
            return '?';
          }).join(',') + ')', nonDeleted, function () {
            onComplete();
          });
        });
      });
  ...
  function setup() {
    ...
    db.transaction(function (tx) {
      ...
      tx.executeSql(doc);
      tx.executeSql(seq);
      tx.executeSql(meta, [], function (tx) {

        // run migration #2 if necessary
        tx.executeSql('SELECT sql FROM sqlite_master WHERE tbl_name = ' + META_STORE, [], function (tx, result) {
          if (!/db_version/.test(result.rows.item(0).sql)) {
            ...
            setVersion(); // db_version column already exists, no need to migrate
          }
        });
      });
