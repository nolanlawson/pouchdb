...
function webSqlPouch(opts, callback) {
  ...
  function decodeUtf8(str) {
    window.console.log('1got str: ' + JSON.stringify(str));
    var result;
    try {
      result = decodeURIComponent(window.escape(str));
      window.console.log('2got str: ' + JSON.stringify(result));
    } catch (err) {
      // occasional safari bug, string is already escaped
      result = str;
      window.console.log('3got str: ' + JSON.stringify(result));
    }
    // Safari is weird, it encodes everything with bonus \u0000 characters... sometimes
    var isSafari = /Safari/.test(window.navigator.userAgent) && !/Chrome/.test(window.navigator.userAgent);
    if (isSafari) {
      result = JSON.parse(JSON.stringify(result).replace(/\\u[0-9a-f]{4}/g, ''));
      window.console.log('4got str: ' + JSON.stringify(result));
    }
    window.console.log('5got str: ' + JSON.stringify(result));
    return result;
  }
  ...
  api._getAttachment = function (attachment, opts, callback) {
    ...
    tx.executeSql(sql, [digest], function (tx, result) {
      // FIXME: sqlite normally stores data as utf8, so even the hex() function "encodes" the binary
