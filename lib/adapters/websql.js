...
var BY_SEQ_STORE_DELETED_INDEX = quote('by-seq-deleted-idx');
var DOC_STORE_WINNINGSEQ_INDEX = quote('doc-winningseq-idx');
...
function webSqlPouch(opts, callback) {
  ...
  // In this migration, we added the 'deleted' column to the the doc store.
  // To preserve existing user data, we re-process all the existing JSON
  // and add this value.
  // Called migration2 because it corresponds to adapter version (db_version) #2
  function runMigration2(tx, onComplete) {

    tx.executeSql('CREATE INDEX IF NOT EXISTS ' + DOC_STORE_WINNINGSEQ_INDEX + ' ON ' + DOC_STORE +
        ' (winningseq)');

    tx.executeSql('ALTER TABLE ' + BY_SEQ_STORE + ' ADD COLUMN deleted TINYINT(1)', [], function () {
      tx.executeSql('CREATE INDEX IF NOT EXISTS ' + BY_SEQ_STORE_DELETED_INDEX + ' ON ' +
          BY_SEQ_STORE + ' (seq, deleted)', [], function () {
        ...
        tx.executeSql(sql, [], function (tx, result) {
          ...
          tx.executeSql('UPDATE ' + BY_SEQ_STORE + ' SET deleted = 1 WHERE seq IN (' + deleted.map(function () {
            return '?';
          }).join(',') + ')', deleted, function () {
            tx.executeSql('UPDATE ' + BY_SEQ_STORE + ' SET deleted = 0 WHERE seq IN (' + nonDeleted.map(function () {
              return '?';
            }).join(',') + ')', nonDeleted, onComplete);
          });
        });
      });
    });
  ...
  // and we can safely mark the db version
  function onSetupComplete(tx) {
  ...
  api._allDocs = function (opts, callback) {
    ...
    db.transaction(function (tx) {
      ...
      var sql = 'SELECT COUNT(' + DOC_STORE + '.id) AS \'num\' FROM ' +
        from + ' WHERE ' + BY_SEQ_STORE + '.deleted = 0';
